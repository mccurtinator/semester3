...
if [ ! -d $sender ]; then
        echo "nok: user $sender does not  exist" > serverout.txt
        exit 2
# if the $receiver does not exist
elif [ ! -d $receiver ]; then
        echo echo "nok: user $receiver does not exist" > serverout.txt
        exit 3
else
        # if the $receiver is not friend od $sender
        if ! grep "^$sender$" "$receiver"/friends > /dev/null; then
                echo "nok: user $sender is not a friend of $receiver" > serverout.txt
                exit 4
        else
		echo $sender[$DATE]: $message >> $receiver/wall
                echo "ok: message posted on $receiver wall" > serverout.txt
        fi
fi
exit 0
